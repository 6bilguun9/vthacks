...
describe("dining planner", () => {
  ...
  it("uses OpenRouter through the provider boundary and recomputes meal totals", async () => {
    ...
    expect(providerCalls).toEqual([{ url: "https://openrouter.ai/api/v1/chat/completions", authorization: "Bearer test-openrouter-key" }]);
    expect(response.statusCode, response.body).toBe(200);
    expect(response.json()).toMatchObject({ source: "openrouter", model: "openrouter/free", weeklyDiningSpendCents: 2100, projectedDiningSpendCents: 2100, remainingDiningBalanceCents: 900 });
  });
});
