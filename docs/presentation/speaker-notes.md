...
- Source implementation is not proof of hosted readiness. Verify the frontend guest session, configured providers, and an actual `ans_remote` response before describing a connected demo. The ANS call path is specific to Coach → Planner; dining is a separate ARC workflow.
